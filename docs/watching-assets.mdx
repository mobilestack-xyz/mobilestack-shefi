...
Shefi includes a default list of popular tokens. Each token in the
...
want to manage and transact with that do not meet the requirements. To
support these tokens well, Shefi implements a deeplink handler so
that a dapp can construct deeplinks that make it easy for their users
to add arbitrary tokens to Shefi.

## Using deeplinks

> This feature is in-development and not fully supported yet!

Shefi's deeplink handler requires the contract address. If your
[ERC-20](https://eips.ethereum.org/EIPS/eip-20) contract implements
`symbol` and `decimals` Shefi will fetch the token symbol and
...
functionality to also support Shefi.
